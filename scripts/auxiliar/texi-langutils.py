...
process_includes = ('-n', '') not in optlist
...
for x in optlist:
    if x[0] == '-o':  # -o NAME   set PO output file name to NAME
        ...
        doclang = f'; documentlanguage: {x[1]}'
...
def process_texi(texifilename, i_blurb, n_blurb, write_skeleton, topfile,
    ...
    try:
        with open(texifilename, 'r', encoding='utf-8') as f:
            texifile = f.read()
        ...
        if write_skeleton:
            with open(os.path.basename(texifilename), 'w', encoding='utf-8') as g:
                subst = globals() | locals()
                ...
                for item in tutu:
                    if item[0] == '*':
                        ...
                        output_file.write(f'_(r"{item[5]}") # @rglos in {printedfilename}' + '\n')
                    elif item[2] == 'menu':
                        g.write('@menu\n')
                    elif item[2] == 'end menu':
                        g.write('@end menu\n\n')
                    elif item[2] == 'documentlanguage':
                        g.write(f'@documentlanguage {doclang}' + '\n')
                    else:
                        space = ' '
                        if item[3].startswith('{') or not item[3].strip():
                            space = ''
                        g.write(f'@{item[2]}{space}{item[3]}' + '\n')
                        if node_just_defined:
                            g.write(f'@translationof {node_just_defined}' + '\n')
                            g.write(n_blurb)
                            node_just_defined = ''
            ...
            for item in toto:
                if item[0] == 'include':
                    ...
                    output_file.write(
                        (
                            (f'# @{item[0]} in {printedfilename}' + '\n_(r"')
                            + item[1].strip().replace('\\', r'\\')
                            + '")\n'
                        )
                    )
...
if make_gettext:
    ...
    with open(node_list_filename, 'w', encoding='utf-8') as node_list:
